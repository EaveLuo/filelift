...
3. Support multiple named profiles across platforms and buckets.
4. Keep secrets encrypted by delegating to the OS keyring.
5. Generate deterministic public URLs from profile metadata and object keys.
...
The config file stores profile metadata only. Credentials are stored under the
...
```text
filelift profile add <name>
filelift profile list
filelift profile use <name>
filelift profile remove <name>
filelift upload <path>
```

Upload options:

- `--profile <name>` overrides the default profile.
...
1. CLI skeleton and profile config.
